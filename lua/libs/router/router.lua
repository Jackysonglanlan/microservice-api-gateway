...
local _P = require "router.path_parser"
...
function _M.add_route(self, method, route, handler)
  if not method or not route or not handler then
    return "method, route, handler can not be empty"
  end
  ...
  while token and not err do
    ...
    if child.nodeType == _nodeType.catchall and token then
      self:throw("`*` shouldn not at the middle of route")
    end
    ::tryAgain::
  end
  if err then
...
function _M.find_route(self, method, path)
  ...
  if not method or not path then
    return nil, params, "method or path can not be empty"
  end
  method = string.lower(method)
...
function _M.run(self, apiPrefix, notfound_handler)
  local method = ngx.req.get_method()
  local uri = ngx.var.uri
  local prefixLen = string.len(apiPrefix)
  uri = string.sub(uri, prefixLen + 1, string.len(uri)) -- 去掉前缀, 这样路由文件在定义路由时，就可以不用加了
  -- utils.log(uri)
