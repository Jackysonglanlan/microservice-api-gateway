...
function _M.new(opts)
  ...
  return setmetatable({start_time = _M.ngx.now(), unit_name = unit_name, 
...
function _M._log(self, color, ...)
  ...
  if color_d[color] then
    ...
    -- _M.ngx.print(format_color(color_d[color]) .. table.concat( logs, " ") .. '\x1b[m')
    _M.ngx.print(table.concat( logs, "<br>"))
  else
    _M.ngx.print(...)
  end
...
function _M._log_standard_head( self )
  ...
  self:_log("default", string.format("%0.3f", _M.ngx.now() - self.start_time), " ")
...
function _M.run(self, loop_count )
  _M.ngx.header["Content-Type"] = 'text/html'
  _M.ngx.print('<pre>')
...
function _M.bench_back_worker( self, lock_key, micro_count )
  ...
  local ok, err = _M.ngx.timer.at(0, _M.bench_once, self, lock, micro_count)
  if not ok then
...
function _M.bench_stastic( self )
  local current_time = _M.ngx.now() - self.start_time
...
function _M.bench_run(self, total_count, micro_count, parallels)
  ...
  for i = 1, total_count / micro_count, parallels do
    ...
    if _M.ngx.now() - last_stastics_time > 2 then
      last_stastics_time = _M.ngx.now()
      self:bench_stastic()
    end
  end
  ...
  _M.ngx.log(_M.ngx.ERR, "bench all done")
end
